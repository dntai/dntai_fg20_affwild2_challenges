...
affwild2_dict[-1] = "NoFace"
affwild2_dict[-2] = "Test"
affwild2_dict[-3] = "None"

# Expr: -1 (NoFace), -2 (Test), -3 (None)
# VA  : -5 (NoFace), -2 (Test), -3 (None)

affwild2_idict= dict(zip(affwild2_name, np.arange(len(affwild2_name))))
affwild2_idict["NoFace"] = -1
affwild2_idict["Test"] = -2
affwild2_idict["None"] = -3
...
class AffWild2Dataset:
    ...
    def load_scheme_emotion(self):
        ...
        filter_train_basic  = np.logical_and(filter_train_basic, self.df_frames["path"]!="")
        ...
        filter_valid_basic  = np.logical_and(filter_valid_basic, self.df_frames["path"]!="")

        self.a_idx_valid = self.a_idx_all[filter_valid_basic]
        self.n_valid     = len(self.a_idx_valid)

        # Test
        filter_test_basic  = np.logical_and(self.df_frames["expr_type"]=="test", self.df_frames["path"]!="")
        
        self.a_idx_test = self.a_idx_all[filter_test_basic]
        self.n_test     = len(self.a_idx_test)

        print("Loading Scheme Emotion: ")
        save_dir = os.path.dirname(self.db_file)
        a_all_scheme = get_index_scheme_emotion(self, has_va=False, save_path=os.path.join(save_dir, "scheme_emotion_v1.pkl"))
    ...
    def load_scheme_emotion_va(self):
        ...
        # Test VA
        filter_test_va_basic  = np.logical_and(self.df_frames["va_type"]=="test", self.df_frames["path"]!="")
        
        self.a_idx_va_test = self.a_idx_all[filter_test_va_basic]
        self.n_va_test     = len(self.a_idx_va_test)

        print("Loading Scheme Emotion VA: ")   
        save_dir = os.path.dirname(self.db_file)
        a_all_scheme = get_index_scheme_emotion(self, has_va=True, save_path=os.path.join(save_dir, "scheme_emotion_va_v1.pkl"))
        self.a_scheme_train = a_all_scheme["train"]
        self.a_scheme_valid = a_all_scheme["valid"]
        self.a_scheme_test = a_all_scheme["test"]
    ...
    def view_image(self, idx_show, a_idx_ranges, a_scheme, n_blocks = 32, 
                   transforms = None, expr_show = True, va_show = False, 
                   title = "View Images", rows=4, cols=8, figsize=(10, 16), verbose = 1):
        # Choose idx show
        if idx_show is None: 
            idx_show = np.random.choice(len(a_idx_ranges), rows * cols)
        a_idx_show = self.get_block_idx(a_idx_ranges[idx_show], a_scheme, n_blocks)
        self.debug_view_images = {}
        self.debug_view_images["idx_show"] = a_idx_ranges[idx_show]
        self.debug_view_images["a_idx_show"] = a_idx_show
        
        if title != "": print(title)
        # View images
        rows = (len(a_idx_show) + cols - 1) // cols
        for row in range(rows):
            plt.figure(figsize=figsize)
            for col in range(cols):
                idx = row * cols + col
                if idx>=len(a_idx_show): break
    ...
    def view_images(self, a_idx_ranges, idx_show = None, transforms = None, 
        ...
        self.debug_view_images = {}
        self.debug_view_images["idx_show"] = idx_show
        self.debug_view_images["a_idx_show"] = a_idx_show
